const toneColorsMapTxtRGB = {
  "-1.0": "rgba(185, 28, 28, 1)", // dark red
  "-0.9": "rgba(200, 60, 60, 1)", // medium dark red
  "-0.8": "rgba(220, 90, 90, 1)", // red
  "-0.7": "rgba(235, 120, 120, 1)", // medium red
  "-0.6": "rgba(250, 150, 150, 1)", // light red
  "-0.5": "rgba(255, 180, 180, 1)", // lighter red
  "-0.4": "rgba(255, 210, 210, 1)", // very light red
  "-0.3": "rgba(255, 235, 235, 1)", // very light pink
  "-0.2": "rgba(255, 245, 245, 1)", // almost white with a hint of pink
  "-0.1": "rgba(255, 250, 250, 1)", // very very light pink
  "-0.0": "rgba(255, 255, 255, 1)", // white
  "0.0": "rgba(255, 255, 255, 1)", // white
  "0.1": "rgba(235, 245, 235, 1)", // very very light green
  "0.2": "rgba(210, 230, 210, 1)", // very light green
  "0.3": "rgba(185, 215, 185, 1)", // light green
  "0.4": "rgba(160, 200, 160, 1)", // lighter green
  "0.5": "rgba(135, 185, 135, 1)", // medium light green
  "0.6": "rgba(110, 170, 110, 1)", // green
  "0.7": "rgba(85, 155, 85, 1)", // medium green
  "0.8": "rgba(60, 140, 60, 1)", // darker green
  "0.9": "rgba(40, 125, 40, 1)", // dark green
  "1.0": "rgba(20, 110, 20, 1)", // very dark green
};

const toneColorsMapTxtRGBDarker = {
  "-1.0": "rgba(185, 28, 28, 1)", // dark red
  "-0.9": "rgba(185, 28, 28, 1)", // medium dark red
  "-0.8": "rgba(200, 60, 60, 1)", // red
  "-0.7": "rgba(220, 90, 90, 1)", // medium red
  "-0.6": "rgba(235, 120, 120, 1)", // light red
  "-0.5": "rgba(250, 150, 150, 1)", // lighter red
  "-0.4": "rgba(255, 180, 180, 1)", // very light red
  "-0.3": "rgba(255, 210, 210, 1)", // very light pink
  "-0.2": "rgba(255, 210, 210, 1)", // almost white with a hint of pink
  "-0.1": "rgba(255, 210, 210, 1)", // very very light pink
  "-0.0": "rgba(255, 255, 255, 1)", // white
  "0.0": "rgba(255, 255, 255, 1)", // white
  "0.1": "rgba(185, 215, 185, 1)", // very very light green
  "0.2": "rgba(185, 215, 185, 1)", // very light green
  "0.3": "rgba(160, 200, 160, 1)", // light green
  "0.4": "rgba(135, 185, 135, 1)", // lighter green
  "0.5": "rgba(110, 170, 110, 1)", // medium light green
  "0.6": "rgba(85, 155, 85, 1)", // green
  "0.7": "rgba(60, 140, 60, 1)", // medium green
  "0.8": "rgba(40, 125, 40, 1)", // darker green
  "0.9": "rgba(20, 110, 20, 1)", // dark green
  "1.0": "rgba(20, 110, 20, 1)", // very dark green
};

export {
  toneColorsMapTxtRGB,
  toneColorsMapTxtRGBDarker
};
